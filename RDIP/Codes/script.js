...
function add(arr){
    ...
    for(i=0;i<arr.length;i++){
        ...
        but.setAttribute("value",arr[i]);
        k += String(parseFloat(k) + 1);
        but.innerHTML = arr[i];
        document.getElementById('experiment-sentence').appendChild(but);
    }
...
function sentence_formed(on){
    if(on == "experiment-sentence")
        return ;
    document.getElementById('msg1').innerHTML = "Formed Sentence ";
    document.getElementById('msg2').innerHTML = "(after selecting words):";
...
function assi(on){
    if(on == "experiment-sentence")
        return ;
    if(word_count == 0){
        document.getElementById('formed-sentence').innerHTML = String( document.getElementById(on).value );
    }
    else{
...
function display_set(val){
    ...
    document.getElementById("msg1").style.display = val;
    document.getElementById("msg2").style.display = val;
}